...
# Actions ------------------------------------------------------------------------------
	...
	if ($action) {
		...
		for ($action) {
			...
			/make_search_forms/ && do { &make_search_forms($dbh,$query); last; };	 #Make search form templates 
...
print "Admin General";
	...
	sub admin_cron {
		...
		my $content = qq|<h3>Cron Status</h3><p>
...
sub make_search_forms($dbh,$query) {
...
print "<p>Making Search forms templates<p>";		
	...
	foreach my $table (@tables) {
		...
		my @optlists = &db_get_record_list($dbh,"optlist",{optlist_table=>"$table"});
		foreach my $optlist (@optlists){ 
			my $options = &db_get_record($dbh,"optlist",{optlist_id=>"$optlist"});
			my $option_field = $options->{optlist_field};
			next unless ($option_field);	# Some optlist entries are incomplete
			...
			foreach my $option (@option_list) {
				...
				$doptions->{$table}->{$option_field}->{$ofield} = $oname;
			}
		}
